  ...
  const getResponseTurbo = async (userInputVal: string, msgCount: number) => {
    ...
    try {
      const res = await axios.get(
        `http://13.235.81.90:2000/api/v1/gpt/response/turbo?userId=${userId}&userInput=${userInputVal}&accessToken=${accessToken}&numberOfMessages=${
          msgCount + 1
        }`
      );
      ...
      axios
        ...
        .then(() => {
          axios
            .get(
              `http://13.235.81.90:2000/api/v1/gpt/belief/davinci?userId=${userId}&accessToken=${accessToken}&numberOfMessages=${
                msgCount + 1
              }`
            )
            .then((response) => {
              ...
              axios
                ...
                .then(() => {
                  axios
                    .get(
                      `http://13.235.81.90:2000/api/v1/gpt/symptom/davinci?userId=${userId}&accessToken=${accessToken}&numberOfMessages=${
                        msgCount + 1
                      }`
                    )
                    .then((response) => {

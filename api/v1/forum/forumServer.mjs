...
import { router } from "./routes/forumRoutes.mjs";
import {
...
app.use("/api/v1/forum", router);

app.use(sendInvalidRouteError);
app.use(sendNoParametersSentError);

const port = process.env.FORUM_API_PORT || 1001;

app.listen(port, () => {
  console.log(`forum server is listening on port ${port}`);
});